...
export default function MembershipApplication() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    if (error) {
      ...
      setSubmitMessage({ type: "success", text: "Application submitted successfully! We'll be in touch." });
      setFormData({ fullName: "", studentNumber: "", email: "", course: "", yearLevel: "1st Year" });
    }
      ...
      <main className="flex-grow relative z-10 pb-20">
              ...
              <div className="h-full bg-white/60 backdrop-blur-xl border border-white rounded-[2.5rem] p-10 shadow-xl hover:shadow-2xl transition-all duration-300">
                ...
                {isLoadingContent ? (
                  ...
                  <ul className="space-y-6 text-lg text-[#141414]/80 font-medium">
                    {pageContent.reminders.length > 0 ? pageContent.reminders.map((reminder, idx) => (
                      <li key={idx} className="flex items-start gap-4">
                        <span className="text-[#eec643] text-2xl mt-[-4px]">✦</span> 
                        <span>{reminder}</span>
                      </li>
                    )) : (
                      <p className="italic text-gray-500">No reminders posted.</p>
                    )}
                  </ul>
                )}
              </div>
        ...
        <section className="py-24 px-6 relative flex flex-col items-center justify-center overflow-hidden">
          ...
          <div className="relative z-10 w-full max-w-5xl text-center">
            ...
            <div className="absolute bottom-10 right-4 lg:right-10 text-8xl text-[#0d21a1]/10 rotate-12 select-none pointer-events-none">"</div>
          </div>
        ...
        <section className="py-12 px-6 relative z-10 flex justify-center">
          ...
          >
            ...
            {submitMessage.text && (
              <div className={`p-4 mb-8 rounded-xl font-bold text-center ${submitMessage.type === 'success' ? 'bg-green-100 text-green-800 border border-green-200' : 'bg-red-100 text-red-800 border border-red-200'}`}>
                {submitMessage.text}
              </div>
            )}
            ...
            <form onSubmit={handleSubmit} className="space-y-6">
              <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                ...
                <div className="flex flex-col">
                  <label className="text-sm font-bold text-gray-500 uppercase tracking-widest mb-2 ml-2">Full Name</label>
                  <input required type="text" name="fullName" value={formData.fullName} onChange={handleChange} placeholder="Juan Dela Cruz" className="px-6 py-4 rounded-2xl bg-white/50 border border-gray-200 focus:border-[#0d21a1] focus:ring-2 focus:ring-[#0d21a1]/20 outline-none font-medium text-[#011638] transition-all" />
                </div>


                <div className="flex flex-col">
                  <label className="text-sm font-bold text-gray-500 uppercase tracking-widest mb-2 ml-2">Student Number</label>
                  <input required type="text" name="studentNumber" value={formData.studentNumber} onChange={handleChange} placeholder="202X-XXXXX" className="px-6 py-4 rounded-2xl bg-white/50 border border-gray-200 focus:border-[#0d21a1] focus:ring-2 focus:ring-[#0d21a1]/20 outline-none font-medium text-[#011638] transition-all" />
                </div>


                <div className="flex flex-col md:col-span-2">
                  <label className="text-sm font-bold text-gray-500 uppercase tracking-widest mb-2 ml-2">Email Address</label>
                  <input required type="email" name="email" value={formData.email} onChange={handleChange} placeholder="juan@example.com" className="px-6 py-4 rounded-2xl bg-white/50 border border-gray-200 focus:border-[#0d21a1] focus:ring-2 focus:ring-[#0d21a1]/20 outline-none font-medium text-[#011638] transition-all" />
                </div>


                <div className="flex flex-col">
                  <label className="text-sm font-bold text-gray-500 uppercase tracking-widest mb-2 ml-2">Course / Program</label>
                  <input required type="text" name="course" value={formData.course} onChange={handleChange} placeholder="BS Computer Science" className="px-6 py-4 rounded-2xl bg-white/50 border border-gray-200 focus:border-[#0d21a1] focus:ring-2 focus:ring-[#0d21a1]/20 outline-none font-medium text-[#011638] transition-all" />
                </div>

                <div className="flex flex-col">
                  <label className="text-sm font-bold text-gray-500 uppercase tracking-widest mb-2 ml-2">Year Level</label>
                  <select name="yearLevel" value={formData.yearLevel} onChange={handleChange} className="px-6 py-4 rounded-2xl bg-white/50 border border-gray-200 focus:border-[#0d21a1] focus:ring-2 focus:ring-[#0d21a1]/20 outline-none font-medium text-[#011638] transition-all appearance-none cursor-pointer">
