...
export default function AddEventForm() {
  ...
  const handleSubmit = async (e: React.FormEvent<HTMLFormElement>) => {
    ...
    if (!title) {
      ...
      setErrorMsg("Full title is required.");
    ...
    if (!shortTitle) {
      setInvalidFields(["short_title"]);
      setErrorMsg("Short title is required.");
    ...
    if (endDateObj < startDateObj) {
      setInvalidFields(["start_date", "end_date"]);
    ...
    if (status === "Completed" && endDateObj >= today) {
      ...
      setErrorMsg(
        "Cannot mark as 'Completed'. End date must be in the past.",
      );
    ...
    if (status === "Upcoming" && startDateObj <= today) {
      ...
      setErrorMsg(
        "Cannot mark as 'Upcoming'. Start date must be in the future.",
      );
    ...
    if (
      ...
      setErrorMsg(
        "Cannot mark as 'Ongoing'. Today's date must fall within the event dates.",
      );
