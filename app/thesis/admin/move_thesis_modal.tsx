...
export default function MoveThesisModal({ thesis, onClose, onMove }: MoveThesisModalProps) {
  const [selectedStatus, setSelectedStatus] = useState(thesis.thesis_status); // to check which status is selected
  ...
  const [rejectionReason, setRejectionReason] = useState(thesis.rejection_reason || ""); // rejection reason
  ...
  const isPastDate = new Date(thesis.thesis_end) < new Date(); // check if end date is in the past
  ...
  const canMoveToStatus = (statusValue: string): { allowed: boolean; reason?: string } => {
    const currentStatus = thesis.thesis_status;
  ...
  const handleStatusChange = (statusValue: string) => {
    ...
    if (!allowed && reason) {
      ...
      // reset
      setSelectedStatus(thesis.thesis_status);
      // clear error
  ...
  const getWarningMessage = () => {
    if (isPastDate) {
      if (thesis.thesis_status === 'archived') {
        return {
          title: "Archived Thesis with Past Date",
          message: "This archived thesis has passed its end date. It cannot be moved to Accepted or Pending until the end date is updated to a future date. Only Rejected or Archived status is available."
        };
      }
      if (thesis.thesis_status === 'rejected') {
        return {
          title: "Rejected Thesis with Past Date",
          message: "This rejected thesis has passed its end date. It cannot be moved to Accepted or Pending until the end date is updated to a future date. Only Rejected or Archived status is available."
        };
      }
      if (thesis.thesis_status === 'pending') {
        return {
          title: "Pending Thesis with Past Date",
          message: "This thesis's end date has passed. It cannot be moved to Accepted. Consider archiving it or updating the end date first."
        };
      }
      if (thesis.thesis_status === 'accepted') {
        return {
          title: "Accepted Thesis with Past Date",
          message: "This accepted thesis has passed its end date. It cannot be moved back to Pending. Only Archived or Rejected status is available."
        };
      }
    }
    return null;
  ...
  return (
    ...
    >
      ...
      <main className="container mx-auto py-8 px-4 max-w-3xl">
        <div className="mb-6">
          <BackButton />
          <h1 className="text-2xl font-oswald font-bold text-[#011638] break-words mt-6">Move Thesis</h1>
        </div>
