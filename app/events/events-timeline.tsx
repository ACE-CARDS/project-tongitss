...
export default function EventsTimeline() {
  ...
  const filteredEvents = events.filter(event => {
    const matchesFilter = activeFilter === "ALL" || 
      (activeFilter === "COMPLETED" ? event.status === "COMPLETED" : 
       activeFilter === "UPCOMING" ? event.status !== "COMPLETED" : 
  ...
  const EventCard = ({ event, isActive, index }: { event: any, isActive: boolean, index: number }) => (
    <div 
      onClick={() => {
        if (isActive) {
    ...
    >
      ...
      <div className={`absolute -bottom-16 -right-16 transition-all duration-500 z-0 pointer-events-none ${isActive ? 'text-[#f8f9fa] scale-110 opacity-100' : 'text-transparent'}`}>
      ...
      <div className="relative z-10 flex justify-between items-start mb-6">
        <div className="flex flex-col">
          <span className={`font-black text-[10px] uppercase tracking-widest ${isActive ? 'text-[#eec643]' : 'text-gray-400'}`}>Date</span>
          <span className={`font-black text-lg leading-tight ${isActive ? 'text-[#0d21a1]' : 'text-gray-400'}`}>{event.date}</span>
        </div>
        <span className={`text-3xl leading-none transition-colors ${isActive ? 'text-[#0d21a1]' : 'text-gray-300'}`}>♠</span>
      </div>
      
      <h3 className="relative z-10 text-2xl font-black text-[#011638] uppercase leading-tight mb-2 line-clamp-3">{event.title}</h3>
      <p className="relative z-10 text-[10px] font-bold text-gray-400 uppercase tracking-tighter mb-4">{event.location}</p>
      ...
      <div className="relative z-10 mt-6 pt-6 border-t border-gray-100">
        <div className={`w-full py-4 rounded-2xl font-black text-[10px] tracking-widest transition-all text-center uppercase ${event.status === 'RSVP OPEN' ? 'bg-[#011638] text-white hover:bg-[#0d21a1]' : 'border border-[#011638] text-[#011638] hover:bg-gray-50'}`}>
  ...
  return (
    <section className="py-12 relative">
      ...
      <div className="max-w-7xl mx-auto px-6 mb-12">
        <div className="flex-grow flex flex-col md:flex-row gap-4 w-full bg-white/30 backdrop-blur-md p-3 rounded-[2rem] border border-white/40 shadow-sm">
          ...
          <div className="relative flex items-center">
            <img src="/assets/logos/ACE CARDS logo.png" className="absolute left-4 w-5 h-5 object-contain z-10 pointer-events-none" alt="ace" />
            <select 
      ...
      <div className="w-full relative mb-16 py-8 overflow-x-auto hide-scrollbar border-y border-gray-100/50 bg-white/10">
        ...
        <div ref={timelineRef} className="relative flex justify-between min-w-max px-[20vw] gap-20">
          {filteredEvents.map((node, index) => (
            <button key={node.id} onClick={() => scrollToCard(index)} className="flex flex-col items-center group relative z-10">
              <div className={`w-3 h-3 rounded-full border-[1.5px] mb-3 transition-all duration-500 ${index === activeIndex ? 'bg-[#eec643] border-[#011638] scale-150' : 'bg-white border-gray-300'}`}></div>
              <span className={`font-black text-[9px] uppercase tracking-tighter ${index === activeIndex ? 'text-[#0d21a1]' : 'text-gray-400'}`}>{node.shortTitle}</span>
            </button>
          ))}
        </div>
      </div>
      ...
      <div className="max-w-7xl mx-auto px-6">
        <div className="flex items-center justify-between mb-8">
          ...
          <div className="flex gap-3">
            <button onClick={() => scroll("left")} className="w-12 h-12 flex items-center justify-center rounded-full border-[1.5px] border-[#011638] text-[#011638] hover:bg-[#011638] hover:text-white transition-all">
              <svg width="20" height="20" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="3" strokeLinecap="round" strokeLinejoin="round">
                <line x1="19" y1="12" x2="5" y2="12" />
                <polyline points="12 19 5 12 12 5" />
              </svg>
            </button>
            <button onClick={() => scroll("right")} className="w-12 h-12 flex items-center justify-center rounded-full border-[1.5px] border-[#011638] text-[#011638] hover:bg-[#011638] hover:text-white transition-all">
              <svg width="20" height="20" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="3" strokeLinecap="round" strokeLinejoin="round">
        ...
        >
          {filteredEvents.map((event, index) => (
            <EventCard key={event.id} event={event} isActive={index === activeIndex} index={index} />
          ))}
