      ...
      {isLoading ? (
        ...
        <>
          ...
          <div className="w-full flex items-center justify-between gap-4 lg:gap-8 mt-2">
            <button onClick={() => scroll("left")} disabled={activeIndex === 0} className="hidden md:flex shrink-0 z-30 w-14 h-14 items-center justify-center rounded-2xl bg-[#011638] text-white hover:bg-[#0d21a1] hover:scale-110 shadow-lg disabled:opacity-0 transition-all duration-300">
            ...
            <div ref={scrollContainerRef} onScroll={handleScroll} className="flex-1 flex gap-8 overflow-x-auto snap-x snap-mandatory py-8 px-4 lg:px-8 [-ms-overflow-style:none] [scrollbar-width:none] [&::-webkit-scrollbar]:hidden scroll-smooth items-center">
              {filteredEvents.length === 0 ? (
                ...
                filteredEvents.map((event, index) => (
                  <div 
                    key={event.id} 
                    onClick={() => { if (index === activeIndex) router.push(`/events/${event.id}`); else scrollToCard(index); }} 
            ...
            <button onClick={() => scroll("right")} disabled={activeIndex === filteredEvents.length - 1 || filteredEvents.length === 0} className="hidden md:flex shrink-0 z-30 w-14 h-14 items-center justify-center rounded-2xl bg-[#011638] text-white hover:bg-[#0d21a1] hover:scale-110 shadow-lg disabled:opacity-0 transition-all duration-300">
