              ...
              <li
                ...
                className={`px-5 py-3 cursor-pointer transition-colors text-sm font-bold font-ubuntu-mono uppercase tracking-widest ${
                  o.value === value
                    ...
                    : "hover:bg-slate-100 text-[#011638]"
                }`}
              >
                {o.label}
              </li>
          ...
          <div className="overflow-x-auto pb-8 pt-12 px-6 [-ms-overflow-style:none] [scrollbar-width:none] [&::-webkit-scrollbar]:hidden">
            <div className="relative min-w-max flex justify-between items-center gap-24 md:gap-40 lg:gap-56 px-8 md:px-16 mx-auto">
              ...
              {["ALL", ...availableYears].map((year) => (
                ...
                >
                  <span
                    className={`absolute -top-10 text-[11px] font-black uppercase tracking-widest whitespace-nowrap transition-colors ${activeYear === year ? "text-[#011638]" : "text-slate-400 group-hover:text-[#0d21a1]"}`}
                  >
                    {year === "ALL" ? "All Years" : year}
                  </span>
                  <div
                    className={`w-6 h-6 rounded-full border-[4px] transition-all duration-300 flex items-center justify-center ${activeYear === year ? "bg-[#eec643] border-[#011638] scale-[1.3] shadow-md" : "bg-white border-slate-300 group-hover:border-[#0d21a1]"}`}
                  ></div>
                </div>
              ))}
            </div>
                ...
                {currentEvents.map((event) => {
                  ...
                  return (
                    <div
                      ...
                      shadow-md transition-all duration-300 ease-out hover:-translate-y-3 hover:shadow-2xl hover:border-indigo-200 
                      hover:bg-white flex flex-col cursor-pointer overflow-hidden h-full w-full"
                    >
                      <div className="relative z-10 flex flex-col h-full w-full">
                        ...
                        <div className="flex flex-col flex-grow p-6 sm:p-8 w-full min-w-0">
                          <div className="flex flex-col mb-4 w-full min-w-0">
                            ...
                            <span className="font-black text-lg leading-tight text-[#0d21a1] break-all line-clamp-2">
                          ...
                          <h3 className="text-2xl font-black text-[#011638] font-oswald uppercase leading-tight mb-2 line-clamp-2 break-all w-full min-w-0">
                            {event.title}
                          </h3>
                          <div className="flex items-center gap-1 mb-4 w-full min-w-0">
                            <span className="shrink-0 text-xs">📍</span>
                            <p className="text-xs font-bold text-slate-500 uppercase tracking-widest truncate flex-1 min-w-0">
                              {event.location}
                            </p>
                          </div>
                          <p className="text-slate-600 font-ubuntu-mono text-sm leading-relaxed flex-grow line-clamp-3 break-words w-full min-w-0">
                            {event.description}
                          </p>
                  ...
                  <div className="flex items-center gap-1 overflow-x-auto max-w-[200px] sm:max-w-none custom-scrollbar pb-2 sm:pb-0">
                    {[...Array(totalPages)].map((_, i) => (
                      <button
                        ...
                        className={`w-10 h-10 shrink-0 rounded-xl font-bold transition-colors ${currentPage === i + 1 ? "bg-[#011638] text-white shadow-md" : "text-[#475569] hover:bg-slate-200"}`}
                      >
                        {i + 1}
                      </button>
                    ))}
              ...
              {selectedEvent.image_url ? (
                <div className="w-full h-64 sm:h-80 bg-slate-100 relative">
                  ...
                  <div className="absolute bottom-6 left-6 right-6 w-[calc(100%-3rem)]">
                    ...
                    <h2 className="text-3xl sm:text-4xl font-black text-white font-oswald uppercase leading-tight drop-shadow-md break-all">
                      {selectedEvent.title}
                    </h2>
                  </div>
                </div>
                ...
                <div className="w-full pt-16 pb-8 px-6 sm:px-10 bg-[#011638] relative">
                  ...
                  <h2 className="text-3xl sm:text-4xl font-black text-white font-oswald uppercase leading-tight break-all">
                    {selectedEvent.title}
                  </h2>
                </div>
              )}
            ...
            <div className="p-6 sm:p-10 flex flex-col gap-6 w-full">
              <div className="flex flex-col sm:flex-row gap-6 sm:gap-12 pb-6 border-b border-slate-100 w-full items-start">
                <div className="w-full sm:w-1/3 min-w-0">
                  ...
                  <p className="font-bold text-[#0d21a1] text-lg font-ubuntu-mono break-all">
                    {formatEventDateRange(
                ...
                <div className="w-full sm:w-1/3 min-w-0">
                  ...
                  <p className="font-bold text-slate-700 text-lg font-ubuntu-mono break-all">
                ...
                <div className="w-full sm:w-1/3 min-w-0">
                  ...
                  <p className="font-bold text-slate-700 text-lg font-ubuntu-mono uppercase break-all">
                    {selectedEvent.status}
                  </p>
                </div>
              </div>
              <div className="w-full min-w-0 pb-4">
                <p className="text-slate-700 font-ubuntu-mono text-base leading-relaxed whitespace-pre-wrap break-all w-full">
