    ...
    return filteredItems.map((item, index) => {
      return (
        <tr key={item.id} className={index % 2 === 0 ? 'bg-white' : 'bg-[#fbfaf8]'}>
          <td className="px-4 py-4 text-center font-bold text-[#011638] text-sm font-ubuntu-mono">
            {item.type === 'video' ? '-' : item.order_index}
          </td>
          ...
          <td className="px-4 py-4 text-sm text-[#475569] font-ubuntu-mono break-words whitespace-normal min-w-0 max-w-full align-top">
            {item.type === 'video' ? (
              <a href={item.description} target="_blank" className="text-[#0d21a1] hover:text-[#011638] underline break-all">
                {item.description}
              </a>
            ) : (
              item.description
    ...
    <div className="px-4 sm:px-6 lg:px-8 py-8 w-full max-w-full overflow-hidden">
        ...
        <div className="flex flex-col sm:flex-row items-center gap-2 w-full xl:w-auto">
          <div className="flex items-center gap-2 w-full sm:w-auto">
            ...
            <input 
              ...
              className="w-full sm:w-auto px-4 py-2 rounded-lg border border-[#011638] focus:outline-none focus:ring-[#011638] bg-[#fbfaf8] text-[#475569] font-ubuntu-mono"
            />
          </div>
          <button 
            onClick={saveDeadline}
            disabled={savingDeadline}
            className="w-full sm:w-auto px-6 py-2 bg-[#011638] text-white rounded-lg hover:bg-[#0d21a1] transition-colors flex items-center justify-center font-oswald uppercase tracking-widest whitespace-nowrap shadow-sm disabled:opacity-50"
          >
      ...
      {deadlineItem && (
        <div className="mb-6 -mt-3 text-left w-full">
          <span className="text-sm text-[#475569] font-ubuntu-mono">
            Currently Displayed As: <strong className="text-red-600 ml-1">{deadlineItem.description}</strong>
          </span>
        </div>
      )}
      ...
      <div className="mb-6 flex flex-col sm:flex-row gap-4 items-center">
        ...
        <select
          ...
          className="w-full sm:w-auto px-4 py-2 border border-[#011638] rounded-lg focus:outline-none focus:ring-[#011638] bg-[#fbfaf8] text-[#475569] font-ubuntu-mono min-w-[160px]"
        >
        ...
        <div className="overflow-x-auto w-full">
          <table className="min-w-full">
            <thead className="bg-[#011638]">
              <tr>
                <th className="px-4 py-3 text-center text-xs font-oswald font-bold text-[#eff0f2] uppercase tracking-wider w-24">Seq</th>
                <th className="px-4 py-3 text-left text-xs font-oswald font-bold text-[#eff0f2] uppercase tracking-wider w-32">Type</th>
                <th className="px-4 py-3 text-left text-xs font-oswald font-bold text-[#eff0f2] uppercase tracking-wider">Content</th>
                <th className="px-4 py-3 text-center text-xs font-oswald font-bold text-[#eff0f2] uppercase tracking-wider w-24">Actions</th>
              </tr>
            </thead>
