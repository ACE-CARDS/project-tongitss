...
import { useUser } from "@/components/context/userContext"; //audit log
...
export default function AnnouncementsAdmin() {
  ...
  const activeTab =
    (searchParams.get("type") as "landing" | "dashboard") || "landing";
  ...
  const logDeleteAudit = async (
    ...
    const detailedMessage = `Deleted announcement "${itemTitle || "Unknown Title"}" (ID: ${recordId || "Unknown ID"}) from ${tableName}`;
  ...
  const fetchAnnouncements = async () => {
    setLoading(true);
    const tableName =
      activeTab === "landing" ? "announce_landing" : "announce_dash";
    const prefix =
      activeTab === "landing" ? "announce_landing" : "announce_dash";
  ...
  const handleDelete = async () => {
    ...
    const tableName =
      activeTab === "landing" ? "announce_landing" : "announce_dash";
    const deletedItem = announcements.find((item) => item.id === selectedId);
    ...
    //audit
    await logDeleteAudit(tableName, selectedId, deletedItem?.title);
    ...
    <div className="px-4 sm:px-6 lg:px-8 py-8">
      ...
      <div className="mb-8 flex justify-between sm:items-end items-center sm:flex-row flex-col sm:gap-0 gap-3">
        ...
        <div className="flex gap-2 p-1 bg-gray-100 w-fit rounded-lg border border-gray-200">
          ...
          <button
            onClick={() => {
              setSearchTerm("");
              updateUrl({ type: "dashboard", page: "1" });
            }}
      ...
      <div className="mb-6 flex flex-col sm:flex-row gap-4 items-center">
        ...
        <Link
          ...
          className="bg-[#eec643] text-[#011638] px-6 py-2 rounded-lg hover:bg-[#d9b237] flex items-center gap-2 font-oswald"
        >
          <svg
            className="w-5 h-5"
            fill="none"
          ...
          <table className="w-full table-fixed">
            <thead className="bg-[#011638]">
              <tr>
                ...
                <th className="w-[40%] px-4 py-3 text-center text-xs font-oswald font-bold text-[#eff0f2] uppercase tracking-wider">
                  Description
                </th>
                <th
                  ...
                  <div className="flex items-center justify-center gap-2">
                    ...
                    <div className="flex flex-col gap-0.5">
                      <svg
                        ...
                        strokeWidth="2"
                        stroke="currentColor"
                        className={`w-3.5 h-3.5 -mb-1 ${sortField === "start_date" && sortOrder === "asc" ? "text-[#eec643]" : "text-[#eff0f2]/30"}`}
                      >
                        <path
                          strokeLinecap="round"
                          strokeLinejoin="round"
                      ...
                      <svg
                        ...
                        strokeWidth="2"
                        stroke="currentColor"
                        className={`w-3.5 h-3.5 -mt-1 ${sortField === "start_date" && sortOrder === "desc" ? "text-[#eec643]" : "text-[#eff0f2]/30"}`}
                      >
                        <path
                          strokeLinecap="round"
                          strokeLinejoin="round"
                ...
                <th className="w-[12%] px-4 py-3 text-center text-xs font-oswald font-bold text-[#eff0f2] uppercase tracking-wider">
                  Actions
                </th>
              </tr>
            </thead>
            <tbody className="divide-y divide-gray-200">
              {loading ? (
                <tr>
                  <td
                    colSpan={4}
                    className="py-10 text-center font-ubuntu-mono text-gray-400"
                  >
                    Loading records...
                  </td>
                </tr>
                ...
                currentItems.map((item, index) => (
                  ...
                  >
                    <td className="px-4 py-4 font-semibold text-[#011638] text-sm break-words">
                      {item.title}
                    </td>
