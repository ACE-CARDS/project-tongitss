...
function SearchBar({ searchTerm, onSearchChange }: { searchTerm: string; onSearchChange: (value: string) => void }) {
  return (
    <div className="relative flex-1">
      ...
      <svg className="w-5 h-5 text-[#011638] absolute left-3 top-1/2 transform -translate-y-1/2" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M21 21l-6-6m2-5a7 7 0 11-14 0 7 7 0 0114 0z" /></svg>
    </div>
  );
}
...
export default function MemAppAdmin() {
  ...
  const fetchItems = async () => {
    setLoading(true);
    try {
      const { data, error } = await supabase.from('announce_memapp').select('*').order('type', { ascending: true }).order('order_index', { ascending: true });
      ...
      if (data) {
        ...
        if (sl) { setSignupLinkItem(sl); setSignupLink(sl.description); }
      }
    } catch (err: any) {
  ...
  const saveDeadline = async () => {
    ...
    try {
      ...
      fetchItems();
    } catch (err: any) { setToast({ message: err.message, type: 'error' }); } finally { setSavingDeadline(false); }
  };
  ...
  const saveSignupLink = async () => {
    ...
    try {
      ...
      fetchItems();
    } catch (err: any) { setToast({ message: err.message, type: 'error' }); } finally { setSavingLink(false); }
  };
      ...
      <div className="bg-white p-5 rounded-xl shadow-sm border border-gray-200 mb-8 flex flex-col gap-4">
        <div className="flex flex-col sm:flex-row items-center justify-between gap-4">
            <div className="flex flex-col sm:flex-row items-center gap-3 w-full lg:w-auto">
                ...
                <input type="date" value={deadlineDate} onChange={(e) => setDeadlineDate(e.target.value)} className="w-full sm:w-auto px-4 py-2 rounded-lg border border-[#011638] bg-[#fbfaf8] text-[#475569] font-ubuntu-mono" />
                <button onClick={saveDeadline} disabled={savingDeadline} className="w-full sm:w-auto px-6 py-2 bg-[#011638] text-white rounded-lg hover:bg-[#0d21a1] font-oswald disabled:opacity-50">Save</button>
            </div>
            {deadlineItem && <span className="text-xs text-[#475569] font-ubuntu-mono">Current: <strong className="text-red-600">{deadlineItem.description}</strong></span>}
        </div>
        ...
        <div className="flex flex-col sm:flex-row items-center gap-3 w-full">
            ...
            <input type="url" value={signupLink} onChange={(e) => setSignupLink(e.target.value)} placeholder="https://..." className="flex-1 px-4 py-2 rounded-lg border border-[#011638] bg-[#fbfaf8] text-[#475569] font-ubuntu-mono" />
            <button onClick={saveSignupLink} disabled={savingLink} className="px-6 py-2 bg-green-600 text-white rounded-lg hover:bg-green-700 font-oswald disabled:opacity-50">Update Link</button>
        </div>
      </div>
        ...
        <div className="overflow-x-auto">
          <table className="min-w-full table-fixed">
            ...
            <tbody className="divide-y divide-gray-200">
              {loading ? (
                <tr><td colSpan={4} className="py-10 text-center font-ubuntu-mono text-[#475569] animate-pulse">Fetching...</td></tr>
              ) : paginatedItems.map((item, idx) => (
                <tr key={item.id} className={`${idx % 2 === 0 ? 'bg-white' : 'bg-[#fbfaf8]'} h-[80px]`}>
                  <td className="px-4 py-2 text-center font-bold text-[#011638] font-ubuntu-mono">
                    {item.type === 'video' ? (item.order_index === 1 ? '🌟' : '🎥') : item.order_index || '•'}
                  </td>
                  <td className="px-4 py-2">
                    <span className={`px-2 py-1 rounded-full text-[10px] font-bold uppercase tracking-tighter ${item.type === 'video' ? 'bg-purple-100 text-purple-700' : item.type === 'reminder' ? 'bg-orange-100 text-orange-700' : 'bg-blue-100 text-blue-700'}`}>
                      {item.type}
                    </span>
                  </td>
                  <td className="px-4 py-2 text-sm font-ubuntu-mono text-[#475569] truncate max-w-xs">{item.description}</td>
                  <td className="px-4 py-2 text-center">
                    <div className="flex justify-center items-center gap-4">
                      {item.type === 'video' && item.order_index !== 1 && (
                        <button onClick={() => handleSetActiveVideo(item.id)} className="text-[10px] bg-[#011638] text-white px-2 py-1 rounded uppercase font-bold hover:bg-[#eec643] hover:text-[#011638] transition-colors">Activate</button>
                      )}
                      <Link href={`/dashboard/edit/mem-app?id=${item.id}`} className="text-[#0d21a1] hover:scale-110 transition-transform"><svg className="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth="1.5" d="m16.862 4.487 1.687-1.688a1.875 1.875 0 1 1 2.652 2.652L10.582 16.07a4.5 4.5 0 0 1-1.897 1.13L6 18l.8-2.685a4.5 4.5 0 0 1 1.13-1.897l8.932-8.931Zm0 0L19.5 7.125M18 14v4.75A2.25 2.25 0 0 1 15.75 21H5.25A2.25 2.25 0 0 1 3 18.75V8.25A2.25 2.25 0 0 1 5.25 6H10" /></svg></Link>
                      <button onClick={() => { setSelectedId(item.id); setDeletePopupOpen(true); }} className="text-red-600 hover:scale-110 transition-transform"><svg className="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth="1.5" d="m14.74 9-.346 9m-4.788 0L9.26 9m9.968-3.21c.342.052.682.107 1.022.166m-1.022-.165L18.16 19.673a2.25 2.25 0 0 1-2.244 2.077H8.084a2.25 2.25 0 0 1-2.244-2.077L4.772 5.79m14.456 0a48.108 48.108 0 0 0-3.478-.397m-12 .562c.34-.059.68-.114 1.022-.165m0 0a48.11 48.11 0 0 1 3.478-.397m7.5 0v-.916c0-1.18-.91-2.164-2.09-2.201a51.964 51.964 0 0 0-3.32 0c-1.18.037-2.09 1.022-2.09 2.201v.916m7.5 0a48.667 48.667 0 0 0-7.5 0" /></svg></button>
                    </div>
                  </td>
                </tr>
              ))}
