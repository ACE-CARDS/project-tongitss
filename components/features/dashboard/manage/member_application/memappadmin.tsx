    ...
    <div className="fixed inset-0 backdrop-blur-[3px] bg-black/30 flex items-center justify-center z-50 p-4">
      <div ref={popupRef} className="bg-[#fbfaf8] rounded-xl max-w-md w-full shadow-2xl overflow-hidden">
        ...
        <div className="px-6 py-6">
          <p className="text-sm text-[#475569] font-ubuntu-mono mb-6">Are you sure you want to delete <span className="font-bold text-[#011638]">&quot;{title}&quot;</span>? This action will hide it from the public view.</p>
          <div className="flex justify-end gap-3">
            <button onClick={onClose} className="px-4 py-2 text-[#475569] font-ubuntu-mono hover:text-[#011638]">Cancel</button>
            <button onClick={onConfirm} className="px-4 py-2 bg-red-600 text-white rounded-lg hover:bg-red-700 font-oswald font-bold">Delete</button>
          </div>
        </div>
      </div>
    </div>
...
function SortableRow({ item, index, isDragEnabled, onActivate, onDelete, onEdit }: { item: MemAppItem, index: number, isDragEnabled: boolean, onActivate: (id: number) => void, onDelete: (id: number) => void, onEdit: (id: number) => void }) {
  ...
  return (
    <tr ref={setNodeRef} style={style} className={`${index % 2 === 0 ? "bg-white" : "bg-[#fbfaf8]"} h-[80px] ${isDragging ? "opacity-50 shadow-2xl bg-blue-50" : ""}`}>
      
      {/* Drag Handle - Replaces Sequence */}
      <td className="px-4 py-2 text-center align-middle w-[5%]">
        {isDragEnabled ? (
          <button {...attributes} {...listeners} className="text-slate-400 hover:text-[#011638] cursor-grab active:cursor-grabbing p-1" title="Drag to reorder">
      ...
      <td className="px-4 py-2 text-center w-[25%]">
        <div className="flex justify-center items-center gap-4">
          {item.type === "video" && item.order_index !== 1 && (
            <button onClick={() => onActivate(item.id)} className="text-[10px] bg-[#011638] text-white px-2 py-1 rounded uppercase font-bold hover:bg-[#eec643] hover:text-[#011638] transition-colors">
              Set Active
            </button>
          )}
          {item.type === "video" && item.order_index === 1 && (
             <span className="text-[10px] bg-green-100 text-green-700 px-2 py-1 rounded uppercase font-bold">Active Video</span>
          )}
          <button onClick={() => onEdit(item.id)} className="text-[#0d21a1] hover:scale-110 transition-transform">
          ...
          <button onClick={() => onDelete(item.id)} className="text-red-600 hover:scale-110 transition-transform">
...
export default function MemAppAdmin() {
  ...
  const ITEMS_PER_PAGE = 8; // Increased slightly since sequence is removed
  ...
  const fetchItems = async () => {
    ...
    try {
      const { data, error } = await supabase
        ...
        .select("*")
        .order("order_index", { ascending: true }); // Reorders globally
  ...
  const saveDeadline = async () => {
    if (!deadlineDate) return setToast({ message: "Please select a date.", type: "error" });
  ...
  const saveSignupLink = async () => {
    ...
    try {
      const { error } = signupLinkItem
        ? await supabase.from("announce_memapp").update({ description: signupLink.trim() }).eq("id", signupLinkItem.id)
        : await supabase.from("announce_memapp").insert([{ type: "signup_link", description: signupLink.trim(), order_index: 0 }]);
      if (error) throw error;
  ...
  const handleSetActiveVideo = async (id: number) => {
    try {
      ...
      await supabase.from("announce_memapp").update({ order_index: 1 }).eq("id", id);
  ...
  const isDragEnabled = activeTab === "instruction" || activeTab === "reminder";
  const totalItems = filteredItems.length;
  const totalPages = Math.ceil(totalItems / ITEMS_PER_PAGE);
  const validCurrentPage = Math.min(Math.max(1, currentPage), totalPages || 1);
  const startIndex = (validCurrentPage - 1) * ITEMS_PER_PAGE;
  
  // Show everything while dragging so you can move items between pages
  const paginatedItems = isDragEnabled ? filteredItems : filteredItems.slice(startIndex, startIndex + ITEMS_PER_PAGE);
    ...
    <div className="px-4 sm:px-6 lg:px-8 py-8 w-full max-w-full overflow-hidden flex flex-col">
      ...
      <div className="mb-6 flex justify-between items-end">
        <div>
          ...
          <p className="text-[#475569] font-ubuntu-mono mt-1">Manage rules, reminders, and video announcements</p>
        </div>
        <Link href="/dashboard/add/mem-app" className="bg-[#eec643] text-[#011638] px-6 py-2.5 rounded-lg hover:bg-[#d9b237] flex items-center justify-center gap-2 font-oswald font-bold shadow-md transition-all">
          <svg className="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2.5} d="M12 4v16m8-8H4" /></svg>
          Add Content
        </Link>
      </div>

      {/* RESTORED: Application Settings (Deadline & Link) Prominently Displayed */}
      <div className="bg-white p-6 rounded-2xl shadow-sm border border-gray-200 mb-8 flex flex-col xl:flex-row gap-8 xl:gap-12">
        ...
        <div className="flex-1">
          <h2 className="text-sm font-oswald font-bold text-[#011638] uppercase tracking-widest mb-3">Application Deadline</h2>
          <div className="flex flex-col sm:flex-row gap-3">
            <input type="date" value={deadlineDate} onChange={(e) => setDeadlineDate(e.target.value)} className="w-full px-4 py-2.5 rounded-lg border border-[#011638] bg-[#fbfaf8] text-[#475569] font-ubuntu-mono focus:ring-1 focus:ring-[#011638]" />
            <button onClick={saveDeadline} disabled={savingDeadline} className="w-full sm:w-auto px-8 py-2.5 bg-[#011638] text-white rounded-lg hover:bg-[#0d21a1] font-oswald tracking-wide disabled:opacity-50 shadow-sm transition-colors">
              Save
            </button>
          </div>
          {deadlineItem && (
             <p className="text-xs text-[#475569] font-ubuntu-mono mt-2">Currently displaying: <strong className="text-red-600 font-bold">{deadlineItem.description}</strong></p>
          )}
        </div>

        {/* Google Form Link Setting */}
        <div className="flex-1">
          <h2 className="text-sm font-oswald font-bold text-[#011638] uppercase tracking-widest mb-3">Google Form Link</h2>
          <div className="flex flex-col sm:flex-row gap-3">
            <input type="url" value={signupLink} onChange={(e) => setSignupLink(e.target.value)} placeholder="https://docs.google.com/forms/..." className="w-full px-4 py-2.5 rounded-lg border border-[#011638] bg-[#fbfaf8] text-[#475569] font-ubuntu-mono focus:ring-1 focus:ring-[#011638]" />
            <button onClick={saveSignupLink} disabled={savingLink} className="w-full sm:w-auto px-8 py-2.5 bg-green-600 text-white rounded-lg hover:bg-green-700 font-oswald tracking-wide disabled:opacity-50 shadow-sm transition-colors">
              Update
            </button>
          </div>
        </div>
        ...
        {TABS.map(tab => (
          ...
          {isDragEnabled && (
            <div className="mb-4 text-sm text-[#0d21a1] font-ubuntu-mono font-bold flex items-center gap-2 bg-blue-50 p-3 rounded-lg border border-blue-100">
              ...
              Reorder Mode Active: Drag the handles on the left to reorder {activeTab === "instruction" ? "announcements" : "reminders"}.
            </div>
          )}
          ...
          <table className="min-w-full table-fixed">
            <thead className="bg-[#011638]">
              <tr>
                ...
                <th className="px-4 py-3 text-left text-xs font-oswald font-bold text-[#eff0f2] uppercase tracking-wider w-[20%]">Type</th>
                <th className="px-4 py-3 text-left text-xs font-oswald font-bold text-[#eff0f2] uppercase tracking-wider w-[50%]">Content Details</th>
                <th className="px-4 py-3 text-center text-xs font-oswald font-bold text-[#eff0f2] uppercase tracking-wider w-[25%]">Actions</th>
              </tr>
            </thead>
            ...
            <DndContext sensors={sensors} collisionDetection={closestCenter} onDragEnd={handleDragEnd}>
                ...
                {loading ? (
                    <tr><td colSpan={4} className="py-10 text-center font-ubuntu-mono text-[#475569] animate-pulse">Fetching records...</td></tr>
                ) : paginatedItems.length === 0 ? (
                    <tr><td colSpan={4} className="py-10 text-center font-ubuntu-mono text-[#475569]">No content found in this category.</td></tr>
                ) : (
                    <SortableContext items={paginatedItems.map(i => i.id.toString())} strategy={verticalListSortingStrategy}>
                        {paginatedItems.map((item, idx) => (
                            <SortableRow 
                                ...
                                onDelete={(id) => { setSelectedId(id); setDeletePopupOpen(true); }}
                                onEdit={(id) => router.push(`/dashboard/edit/mem-app?id=${id}`)}
                            />
                        ))}
                    </SortableContext>
                )}
      ...
      {!isDragEnabled && totalPages > 1 && (
        <div className="mt-6">
            <Pagination currentPage={validCurrentPage} totalPages={totalPages || 1} totalItems={totalItems} itemsPerPage={ITEMS_PER_PAGE} onPageChange={(page) => setCurrentPage(page)} />
        </div>
      )}

      <DeleteConfirmPopup isOpen={deletePopupOpen} onClose={() => setDeletePopupOpen(false)} onConfirm={handleDelete} title={items.find((i) => i.id === selectedId)?.type || "item"} />
    </div>
  );
}