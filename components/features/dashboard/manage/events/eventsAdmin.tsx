    ...
    <div className="fixed inset-0 backdrop-blur-[3px] bg-black/30 flex items-center justify-center z-50">
      <div ref={popupRef} className="bg-[#fbfaf8] rounded-xl max-w-md w-full mx-4 shadow-2xl overflow-hidden">
        ...
        <div className="px-6 py-6">
          <p className="text-sm text-[#475569] font-ubuntu-mono mb-6">Are you sure you want to delete this event? This action cannot be undone.</p>
...
export default function EventsAdmin() {
  ...
  if (loading) return <div className="text-center py-12"><p className="text-gray-500 font-ubuntu-mono">Loading events...</p></div>;
    ...
    <div className="px-4 sm:px-6 lg:px-8 py-8">
      ...
      {filteredEvents.length === 0 ? (
        <div className="text-center py-12 bg-[#fbfaf8] rounded-xl shadow-lg border border-gray-200">
          ...
          <p className="text-gray-500 font-ubuntu-mono">Try adjusting your search or filters.</p>
        </div>
      ) : (
        <>
              ...
              <table className="w-full table-fixed">
                <thead className="bg-[#011638]">
                  <tr>
                    ...
                    <th className={`px-4 py-3 text-center text-xs font-oswald font-bold text-[#eff0f2] uppercase tracking-wider cursor-pointer hover:bg-[#0d21a1] transition-colors w-[220px] ${sortField === "start_date" ? "bg-[#0d21a1]" : ""}`} onClick={() => handleSort("start_date")}>
                      <div className="flex items-center justify-center gap-2">
                        Schedule & Location
                        <div className="flex flex-col gap-0.5">
                    ...
                    <tr key={item.id} className={index % 2 === 0 ? "bg-white" : "bg-[#fbfaf8]"}>
                      <td className="px-4 py-4 text-center">
                        <div className="flex justify-center">
                          <img src={item.image_url || "/assets/logos/ACE CARDS logo.png"} alt="Event" className="w-20 h-20 object-cover rounded-md border border-gray-100" />
                        </div>
                      </td>
                      <td className="px-4 py-4 align-top break-words max-w-full">
                        <div className="text-sm font-oswald font-semibold text-[#011638] mb-1">{item.title}</div>
                        {item.short_title && <div className="text-xs text-gray-500 font-ubuntu-mono mb-1">{item.short_title}</div>}
                        <EventDescription description={item.description} />
                      </td>
                      <td className="px-4 py-4 text-center">
                        <div className="text-sm font-bold text-[#011638] font-ubuntu-mono mb-1">{formatSchedule(item.start_date, item.end_date)}</div>
                        <div className="text-xs text-gray-500 font-ubuntu-mono flex items-center justify-center gap-1">
                            <svg className="w-3 h-3" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth="2" d="M17.657 16.657L13.414 20.9a1.998 1.998 0 01-2.827 0l-4.244-4.243a8 8 0 1111.314 0z" /><path strokeLinecap="round" strokeLinejoin="round" strokeWidth="2" d="M15 11a3 3 0 11-6 0 3 3 0 016 0z" /></svg>
                            {item.location}
                        </div>
                      </td>
                      <td className="px-4 py-4 text-center">
                        <span className={`inline-block px-2 py-1 rounded text-[10px] font-bold tracking-widest uppercase ${item.status?.toUpperCase() === "COMPLETED" ? "bg-gray-100 text-gray-500" : "bg-green-100 text-green-700"}`}>
                          {item.status}
                        </span>
                      </td>
                      <td className="px-4 py-4 text-center">
                        <div className="flex justify-center gap-2">
                          <button onClick={() => router.push(`/dashboard/edit/event/${item.id}`)} className="text-[#0d21a1] hover:scale-110 transition-transform"><svg className="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth="1.5" d="m16.862 4.487 1.687-1.688a1.875 1.875 0 1 1 2.652 2.652L10.582 16.07a4.5 4.5 0 0 1-1.897 1.13L6 18l.8-2.685a4.5 4.5 0 0 1 1.13-1.897l8.932-8.931Zm0 0L19.5 7.125M18 14v4.75A2.25 2.25 0 0 1 15.75 21H5.25A2.25 2.25 0 0 1 3 18.75V8.25A2.25 2.25 0 0 1 5.25 6H10" /></svg></button>
                          <button onClick={() => { setSelectedEvent(item); setDeletePopupOpen(true); }} className="text-red-600 hover:scale-110 transition-transform"><svg className="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth="1.5" d="m14.74 9-.346 9m-4.788 0L9.26 9m9.968-3.21c.342.052.682.107 1.022.166m-1.022-.165L18.16 19.673a2.25 2.25 0 0 1-2.244 2.077H8.084a2.25 2.25 0 0 1-2.244-2.077L4.772 5.79m14.456 0a48.108 48.108 0 0 0-3.478-.397m-12 .562c.34-.059.68-.114 1.022-.165m0 0a48.11 48.11 0 0 1 3.478-.397m7.5 0v-.916c0-1.18-.91-2.164-2.09-2.201a51.964 51.964 0 0 0-3.32 0c-1.18.037-2.09 1.022-2.09 2.201v.916m7.5 0a48.667 48.667 0 0 0-7.5 0" /></svg></button>
                        </div>
                      </td>
                    </tr>
                  ))}
