...
export default function Province({id}: {id?: string}) {
  ...
  // 1. Optimized Data Fetching (Reduces 20+ queries to just 2)
  useEffect(() => {
    const fetchProvinceData = async () => {
      let schoolData;

      // Fetch schools and their member counts in ONE query
      ...
      if (selectedProvince) {
        ...
        if (prov) query.eq("province", prov.id);
      }
